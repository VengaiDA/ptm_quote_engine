 ...
 */
const CACHE_NAME = 'ptm-quote-engine-2026-09-09-p0-3';
const TERMS_PATCH_SCRIPT = './root-terms-patch.js?build=2026.09.09-p0.3';
const APP_SHELL = [
  './',
  './index.html',
...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    // Existing installed/root PWA clients may still be executing the previous
    // index.html. Navigate them once after this worker activates so the fresh
    // navigation passes through injectTermsPatch immediately.
    const clients = await self.clients.matchAll({ type: 'window', includeUncontrolled: true });
    await Promise.all(clients.map(async (client) => {
      try {
        const url = new URL(client.url);
        if (url.origin === self.location.origin && !url.pathname.includes('/agent-lite/')) {
          await client.navigate(client.url);
        }
      } catch (_) {
        // A client may disappear while the service worker is activating.
      }
    }));
  })());
});
